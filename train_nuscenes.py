...
from datasets.DenoisingDatasets_nuscenes import BenchmarkTrain
...
def save_img(im_noisy,output_red,im_restore,mse_img,r2,global_step,psnr):
...
def train(config):
    ...
    train_dataset = BenchmarkTrain(noise_dir, N * batch_size, pch_size=256)
    ...
    for epoch in range(cur_epoch, epochs):
        ...
        for ii, data in enumerate(train_loader):
            ...
            input_red = data[1].to(device)
            input_blue = data[2].to(device)

            mask_red = data[3].to(device)
            mask_blue = data[4].to(device)
