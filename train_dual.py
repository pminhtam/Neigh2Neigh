...
def save_img(im_noisy,im_gt,denoise_red, noise_red, denoise_blue, noise_blue,im_restore,global_step):
    ...
    # plt.show()
    folder = "img_dual_345_gan_denoise_fourier/"
    ...
    for epoch in range(cur_epoch, epochs):
        ...
        for ii, data in enumerate(train_loader):
            ...
            for _ in range(1):
                ...
                # # print(im_restore.size())
                # # print(denoise_red)
                # # print(noise_red)
                # fake_red = torch.clamp(denoise_red + noise_blue, 0, 1)
                # fake_blue = torch.clamp(denoise_blue + noise_red, 0, 1)
                # # print(fake_red.size())
                # # print(torch.cat([fake_red,fake_blue]).size())
                # out_dis = model_dis(torch.cat([fake_red,fake_blue]))
                # # print(out_dis.size())
                # label_dis = torch.cat([torch.zeros(2*len(im_noisy))]).unsqueeze(1).to(device)
                # # print(label_dis.size())
                ...
                # # fake_red = torch.clamp(denoise_red + noise_blue, 0, 1)
                # # fake_blue = torch.clamp(denoise_blue + noise_red, 0, 1)
                # # output_fake_red = net(fake_red)
                # # output_fake_blue = net(fake_blue)
                #
                # # sure 2
                # fake_red = torch.clamp(input_red + noise_blue, 0, 1)
                # fake_blue = torch.clamp(input_blue + noise_red, 0, 1)
                # output_fake_red = net(fake_red)
                # output_fake_blue = net(fake_blue)
                #
                #
                # denoise_fake_red = torch.clamp(output_fake_red[:, :4, :, :], 0, 1)
                # noise_fake_red = torch.clamp(output_fake_red[:, 4:, :, :], -1, 1)
                # denoise_fake_blue = torch.clamp(output_fake_blue[:, :4, :, :], 0, 1)
                # noise_fake_blue = torch.clamp(output_fake_blue[:, 4:, :, :], -1, 1)
                #
                # loss_fake = lossfunc_fake(input_red,noise_blue,input_blue,noise_red,denoise_fake_red,noise_fake_red,denoise_fake_blue,noise_fake_blue)

                ##########################################################
                ########## FourierLoss ###################################
                fourier = loss_fourier(noise_red,noise_blue)
                ########################################################
                # print("loss",loss)
                # Optimization
                # loss += 0.5*loss_fake
            ...
            #     noise_fake_blue = torch.clamp(output_fake_blue[:, 4:, :, :], -1, 1)
            #
            #     loss_fake = lossfunc_fake(denoise_red,noise_blue,denoise_blue,noise_red,denoise_fake_red,noise_fake_red,denoise_fake_blue,noise_fake_blue)
            #
            #     optimizer.zero_grad()
            #     loss_fake.backward()
            #     optimizer.step()
            ...
            #
            #     fake_red = torch.clamp(denoise_red + noise_blue, 0, 1)
            #     real_red = torch.clamp(denoise_red + noise_red, 0, 1)
            #     fake_blue = torch.clamp(denoise_blue + noise_red, 0, 1)
            #     real_blue = torch.clamp(denoise_blue + noise_blue, 0, 1)
            #
            #
            #     out_dis = model_dis(torch.cat([input_blue, input_red, fake_red,fake_blue,real_blue,real_red]))
            #     label_dis = torch.cat([torch.zeros(2*len(input_blue)),torch.ones(2*len(fake_red)),torch.zeros(2*len(input_blue))]).unsqueeze(1).to(device)
            #     loss_ = loss_diss(out_dis,label_dis)
            #
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_dir', type=str, default="img_dual_345_gan_fourier/")
